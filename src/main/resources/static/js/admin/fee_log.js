...
function loadLogs() {
    ...
    // Spring Data JPA는 기본적으로 0페이지부터 시작하므로 currentPage - 1을 전송합니다.
    ...
    fetch(`/admin/fee-logs?page=${requestPage}&size=${pageSize}&severity=${severity}&keyword=${keyword}`)
        ...
        .then(data => {
            ...
            if (!data.content || data.content.length === 0) {
                tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding:30px; color:#999;">조회된 변경 이력이 없습니다.</td></tr>`;
                renderPagination(0);
                return;
            }
...
function renderPagination(totalP) {
    ...
    html += `<button class="btn btn-secondary btn-xs" ${currentPage === totalP ? 'disabled' : `onclick="goToPage(${currentPage + 1})"`}>&gt;</button>`;
...
function saveLog() {
    ...
    saveBtn.disabled = true; // 중복 클릭 방지 비활성화
    ...
    fetch('/admin/fee-logs', {
        ...
        headers: {
            ...
            // 'X-CSRF-TOKEN': document.querySelector('meta[name="_csrf"]').content // Spring Security CSRF 적용 시 주석 해제
        },
    ...
    .then(res => {
        ...
        if (res.ok) {
            ...
            closeLogModal();
            searchLogs(); // 최신 데이터로 리스트 다시 불러오기
        } else {
