/**
 * =========================================================
 * 커뮤니티 시설 예약 및 프로그램 신청 전용 JS (통합 및 병합 완료)
 * =========================================================
 */
...
// [2. 설정] 시설 요금 데이터 (병합 반영)
const guestRoomData = {
    'A': { name: 'A타입 (원룸형/8평)', weekday: 40000, weekend: 60000 },
    'B': { name: 'B타입 (투룸형/15평)', weekday: 60000, weekend: 80000 },
    'C': { name: 'C타입 (스위트/24평)', weekday: 80000, weekend: 100000 }
};
...
function calculateGuestPrice() {
    ...
    while (currentDate < end) {
        ...
        if (day === 5 || day === 6) totalPrice += selectedGuestRoom.weekend; 
        else totalPrice += selectedGuestRoom.weekday; 
...
function refreshGolfAvailability() {
    const grid = document.getElementById('golfGrid'); 
    if(!grid) return;
    grid.innerHTML = '';
    for (let i = 1; i <= 5; i++) {
...
function submitReservation(type, detail, dateStr, priceStr) {
    ...
    const requestData = { facilityType: type, details: detail, reserveDate: dateStr, price: priceStr };
...
function cancelReservation(id, type) {
    if(confirm('정말 취소하시겠습니까?')) {
        fetch(`/api/reservation/cancel/${type}/${id}`, { method: 'POST' })
        .then(res => {
            if(res.ok) {
                alert('취소가 완료되었습니다.');
                location.reload();
...
function displayHistoryPage(page) {
    ...
    if (rows.length === 0 || (rows.length === 1 && rows[0].innerText.includes('없습니다'))) return;
