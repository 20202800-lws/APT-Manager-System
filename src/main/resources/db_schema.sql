...
-- 22. 어린이집 활동 갤러리 (DAYCARE_GALLERY)
CREATE TABLE DAYCARE_GALLERY (
    gallery_id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '갤러리번호',
    writer_id VARCHAR(50) COMMENT '작성자(선생님/관리자)',
    title VARCHAR(200) NOT NULL COMMENT '제목',
    content TEXT COMMENT '내용',
    views INT DEFAULT 0 COMMENT '조회수',
    reg_date DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '등록일',
...
-- 23. 어린이집 활동 갤러리 댓글 (GALLERY_COMMENT)
CREATE TABLE GALLERY_COMMENT (
    comment_id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '댓글번호',
    gallery_id BIGINT NOT NULL COMMENT '갤러리번호',
    user_id VARCHAR(50) NOT NULL COMMENT '작성자(학부모)',
    content VARCHAR(500) NOT NULL COMMENT '내용',
    reg_date DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '등록일',
    FOREIGN KEY (gallery_id) REFERENCES DAYCARE_GALLERY(gallery_id) ON DELETE CASCADE,
    FOREIGN KEY (user_id) REFERENCES USERS(user_id) ON DELETE CASCADE
);
