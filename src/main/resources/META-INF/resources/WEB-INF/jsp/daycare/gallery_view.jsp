            ...
            <div class="content-box">
                <div class="detail-header" style="border-bottom:1px solid #eee; padding-bottom:15px;">
                    ...
                    <div style="color:#888; font-size:14px; margin-top:10px;">
                        선생님: <span style="font-weight:700;">${post.author}</span> | 
                    ...
                    <c:if test="${not empty post.images}">
                        <c:forEach var="img" items="${post.images}">
                            <div style="margin-bottom: 20px;">
                                <img src="<c:url value='/images/${img.saveName}'/>" alt="${img.originalName}" style="max-width:100%; border-radius:12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                            </div>
                        </c:forEach>
                    </c:if>
                ...
                <div style="text-align:right; margin-top:30px; border-top:1px solid #eee; padding-top:20px;">
                    ...
                    <c:if test="${sessionScope.loginMember.userRole == 'ADMIN' || sessionScope.loginMember.userRole == 'TEACHER'}">
                        <span id="myButtons">
                            <form action="<c:url value='/daycare/gallery/delete'/>" method="post" style="display:inline-block; margin-left:5px;" onsubmit="return confirm('사진을 삭제하시겠습니까?');">
