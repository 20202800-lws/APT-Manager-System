					...
					<div class="content-box">
						<div class="detail-header" style="border-bottom:1px solid #eee; padding-bottom:15px;">
							...
							<div style="color:#888; font-size:14px; margin-top:10px;">
								작성자: <span style="font-weight:700;">${post.author}</span> | 날짜:
								<span>${post.regDate}</span> | 조회수: <span>${post.views}</span>
							</div>
						</div>

						<div
							style="margin: 40px 0; min-height:200px; line-height:1.8; white-space: pre-wrap; font-size:16px;">
							${post.content}</div>

						<c:if test="${not empty attachments}">
							<div class="attachment-gallery"
								style="display:flex; gap:15px; flex-wrap:wrap; margin-bottom:40px; background:#f9fafb; padding:20px; border-radius:10px;">
								<h5
									style="width: 100%; margin-top: 0; margin-bottom: 15px; color: #64748b; font-size: 14px;">
								...
								<c:forEach var="comment" items="${comments}">
									<div class="comment-unit" style="border-bottom:1px solid #f1f1f1; padding:15px 0;">
										<div class="comment-info" style="margin-bottom:8px;">
											<b>${comment.author}</b>
											<span
												style="color:#bbb; margin-left:10px; font-size:13px;">${comment.regDate}</span>

											<c:if test="${comment.owner || loginMember.userRole == 'ADMIN'}">
												<form action="<c:url value='/board/comment/delete'/>" method="post"
													...
													<button type="submit"
														style="background:none; border:none; color:#e74c3c; font-size:12px; cursor:pointer;"
														onclick="return confirm('댓글을 삭제할까요?');">삭제</button>
												</form>
											</c:if>
										</div>
										<div style="font-size:15px; color:#333; line-height:1.5;">${comment.content}
										</div>
									</div>
								</c:forEach>

								<c:if test="${empty comments}">
									<p style="text-align:center; color:#ccc; padding:30px;">아직 댓글이 없습니다. 첫 댓글을 남겨주세요!
									</p>
								</c:if>
							</div>

							<form action="<c:url value='/board/comment'/>" method="post"
								style="margin-top:20px; background:#f8fafc; padding:20px; border-radius:8px;">
								<input type="hidden" name="boardId" value="${post.boardId}">
								<input type="hidden" name="boardType" value="free">
								<textarea name="content" placeholder="따뜻한 댓글을 남겨주세요..." required
									style="width:100%; height:80px; padding:12px; border:1px solid #ddd; border-radius:6px; resize:vertical; margin-bottom:10px;"></textarea>
						...
						<div style="text-align:right; margin-top:30px; border-top:1px solid #eee; padding-top:20px;">
							<button type="button" class="btn-sub" onclick="location.href='<c:url value="
								/board/free" />'">목록으로</button>
							<c:if test="${post.owner || loginMember.userRole == 'ADMIN'}">
								<span id="myButtons">
									<button type="button" class="btn-main" style="margin-left:5px;"
										onclick="location.href='<c:url value="
										/board/free/edit?id=${post.boardId}" />'">수정</button>
